...
2. 손님이 폰에서 모바일 신분증 QR 을 띄우면(정부 앱·삼성월렛·네이버·토스·카카오뱅크·KB·NH 등) 검증앱으로 찍는다.
   검증앱이 진짜인지 확인하고 생년월일을 보여 준다. 사진과 얼굴을 대조한다.
   PASS 앱 안의 신분증이면 직원 폰 PASS 앱 → 'QR인증'으로 찍는다.
...
- 손님 폰에 뜬 "확인 결과" 화면은 믿지 않는다. 찍는 쪽은 언제나 직원 폰이다.
- 실물도 모바일 신분증도 없으면 이 방법으로는 확인할 수 없다(모바일 신분증은 실물 IC 카드를 대거나 주민센터·면허시험장에 가야 발급된다).

성인 기준은 만 나이가 아니라 연 나이다(청소년보호법 제2조 제1호 — "만 19세가 되는 해의 1월 1일을 맞이한 사람은 제외").
2026년이면 2007년생까지 통과한다. 판정은 서버 시간대가 아니라 한국 시각으로 센다 — `src/lib/identity/adult.ts`.
모바일 주민등록증은 법에서 실물과 같은 신분증이다(청소년보호법 제29조④ "주민등록증(모바일 주민등록증을 포함한다)").
