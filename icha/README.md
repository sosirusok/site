...
사장님 포스터(`public/images/event/poster.jpg`)가 화면 아래로 그대로 이어진다. 포스터의 밤거리 보케 한 장이 배경으로 고정되고, 그 위에 포스터에서 오려 낸 조각(간판·혜택·리본·순서·메모)이 기울어진 스티커로, 진짜 매장 사진은 폴라로이드로, 안내·쿠폰·입력은 크림색 종이로 붙는다. 글자는 제목·간판·버튼 Do Hyeon, 손글씨 메모 Nanum Pen Script, 본문 Pretendard — 앞의 두 글꼴은 `src/app/fonts.css` 에서 자체 호스팅한다. 버튼은 노란 스티커이고, **초록은 가게마다 네이버 예약하기 하나뿐**이다. 터치 44px, 가로 스크롤 없음. 규칙은 `docs/design.md` 2절.

**문구 톤(v10)**: 손님 화면의 글은 네이버 스마트플레이스·배달의민족·캐치테이블 같은 실제 매장 페이지처럼 쓴다 — 정보는 명사구 또는 합니다체(영업시간 · 주소 · 특별 혜택 · 이용 방법 · 자주 묻는 질문), 상태 칩은 영업 중 / 영업 전 · 17:00 오픈 / 영업 종료 / 휴무, 거리는 도보 1분, 혜택은 "다른 매장 쿠폰 제시 시 {품목} 무료", 버튼은 명사(예약하기 · 쿠폰함 · 사용하기 · 길찾기). 해요체·느낌표·이모지·"여기서/다른 집" 같은 잡담, 사진 캡션은 쓰지 않는다. 놀이 문구는 사장님 포스터 조각(이미지) 안에만 남긴다. 손글씨(Nanum Pen Script)는 크림 종이 위 6단어 이하 짧은 강조에만 쓰고, 영업시간·혜택·규칙·안내문 같은 정보 줄은 Pretendard 15~16px 로 종이나 어두운 띠(#14111a 85% 이상) 위에 놓는다. 보케 위에 바로 놓는 글자는 흰 Do Hyeon 20px 이상 + 검은 2px 외곽선(`.outl`)뿐이고, 노란 글자는 어두운 바탕에서만 쓴다. 문장은 칸 안에서 줄바꿈하고 `nowrap` 으로 잘라내지 않는다.
...
| | 첫 화면 판 | 휴대폰 본인확인 |
|---|---|---|
| 언제 | 첫 방문 1회 (180일 기억) | 쿠폰함에 들어갈 때 |
| 확인하는 것 | 손님이 말한 나이 (고지) | 통신사가 확인한 실제 생년월일 |
| 비용 | 0원 | 건당 과금 (PG 계약) |
| 지금 상태 | **켜짐** | 키 넣으면 켜짐 |

구경만 하는 사람에게 돈이 드는 인증을 요구하지 않고, 공짜 술을 받아 가는 자리에서만 진짜로 확인합니다.
실제로 국내 주류 사이트(오비맥주·카스·국순당)를 열어 확인해 보면 통신사 창을 띄우는 곳은 한 곳도 없습니다 —
통신사 창은 술을 온라인으로 파는 곳이 씁니다. 이 사이트는 팔지 않습니다(쿠폰도 술도 매장 카운터에서 직원이 줍니다).

### 첫 화면 판

`src/components/site/AgeGate.tsx`. `ADULT_GATE` 환경변수로 `year`(기본) / `yesno` / `off`.

판은 늘 HTML 에 들어 있고, 이미 확인한 손님은 판보다 먼저 도는 인라인 스크립트가 `html` 에 `age-ok` 를 붙여
CSS 가 감춥니다 — 그래서 재방문에 깜빡임이 없습니다(측정해서 확인). 기본값을 "보임"으로 둔 이유는
스크립트가 아예 안 돌면(JS 꺼짐) 판이 덮인 채로 남아야 맞기 때문입니다. 관리자 화면에는 붙지 않습니다.

## 휴대폰 본인확인 (성인인증)

통신사를 고르고 이름·생년월일·번호를 넣는 **그 창을 그대로 띄운다.** 비슷한 화면을 우리가 만든 것이 아니라,
본인확인기관(다날 / NHN KCP / KG이니시스)이 띄우는 창이다. 포트원(PortOne) V2 를 통해 호출한다.

**환경변수 세 개가 비어 있으면 기능이 꺼진 채로 동작한다** — 로그인은 지금처럼 번호 입력만으로 되고, 아무것도 달라지지 않는다.
세 개가 채워지는 순간 로그인 화면이 본인확인 한 번으로 바뀌고, 쿠폰함·쿠폰 상세·사용 매장 고르기가 성인 확인을 통과한 사람에게만 열린다.

### 켜는 법

1. [admin.portone.io](https://admin.portone.io) 가입 (사업자등록증 필요)
2. **결제 연동 → 채널 관리**에서 본인인증 채널을 추가하고 다날 / NHN KCP / KG이니시스 중 한 곳과 계약한다.
   계약·심사는 그 PG사가 한다. 심사에 이 사이트 주소를 낸다.
3. 발급된 값 세 개를 Vercel 환경변수에 넣는다. 저장하면 Vercel 이 알아서 다시 배포한다.
   - `PORTONE_STORE_ID` — 연동 정보 화면 오른쪽 위
   - `PORTONE_CHANNEL_KEY` — 채널 관리 탭
   - `PORTONE_API_SECRET` — API Secret
   - `PORTONE_IDENTITY_PG` — `danal` / `kcp` / `inicis` 중 계약한 곳 (다날이면 인증창 자체가 만 19세 미만을 막는다)

### PG 선택이 바꾸는 것

| | 전화번호 | 통신사 | CI/DI | 만나이 제한 |
|---|---|---|---|---|
| 다날 | 별도 계약해야 받음 | 별도 계약 | 항상 | 인증창에서 막아 줌(`AGELIMIT`) |
| NHN KCP | 항상 | 항상 | 항상 | 우리 쪽에서 판정 |
| KG이니시스 통합인증 | 항상 | 없음 | CI만 | 우리 쪽에서 판정 |

전화번호를 받으면 **번호를 손으로 칠 필요가 없다** — 통신사가 확인해 준 번호가 그대로 계정이 된다.
안 받는 계약이면 성인 확인만 끝내고 번호는 따로 받는다(화면이 알아서 그렇게 흐른다).

### 저장하는 것 / 안 하는 것

받는 값은 이름·생년월일·성별·통신사·외국인 여부·CI·DI 지만, **남기는 것은 두 개뿐이다.**

- `members.adult_verified_at` — 성인 확인 시각
- `members.identity_key` — DI 를 서버 비밀키와 함께 해시한 값. 한 사람이 번호만 바꿔 계정을 늘리는지 볼 때만 쓴다. 되돌릴 수 없다.

이름·생년월일·성별·통신사·CI 는 판정에만 쓰고 버린다. 주민등록번호는 애초에 받지 않는다.

성인 기준은 **만 나이가 아니라 연 나이**다(청소년보호법 제2조 제1호: "만 19세가 되는 해의 1월 1일을 맞이한 사람은 제외").
2026년이면 2007년생부터 통과한다. 판정은 서버 시간대가 아니라 한국 시각(UTC+9)으로 센다 — `src/lib/identity/adult.ts`.

### 왜 이렇게 막았나

- 브라우저가 "인증됐다"고 말하는 것은 믿지 않는다. 서버가 포트원에 직접 다시 물어 `status === "VERIFIED"` 인지 본다.
- 인증 건 ID 는 주소창에 그대로 보인다. 그래서 시작할 때 서명된 짧은 쿠키에 적어 두고, 돌아왔을 때 같은 것인지 대조한다. 남의 인증을 주워 쓸 수 없다.
- 화면에 "번호만 넣고 들어가기" 같은 빠져나갈 문을 두지 않았다. 옆에 그 버튼이 있으면 그건 성인 확인이 아니다.
- `/api/auth/login` 도 막았다. 화면을 거치지 않고 이 주소로 바로 쏘는 것으로 확인을 건너뛸 수 있으면 의미가 없다.

로컬에서 흐름만 보고 싶으면 `PORTONE_IDENTITY_MOCK=1` 로 `npm run dev`. `NODE_ENV=production` 이면 무시되므로
Vercel 의 운영·미리보기 배포에서는 절대 켜지지 않는다.

## 운영 주의
- **본인확인을 붙이기 전에는 전화번호만으로 로그인**한다(인증번호 없음 — 사장님 결정). 그래서 남의 번호를 아는 사람이 그 번호로 들어와 쿠폰함을 보고, 길게 눌러 쿠폰을 "사용" 처리해 버릴 수 있다. 완화책: 번호당 로그인 1시간 10회·IP당 10분 30회 제한, 쿠폰 사용은 매장 직원 앞에서만 인정(직원은 관리자 → 쿠폰 조회에서 코드로 확인하고 사용 처리하는 것을 기본 동선으로). **위의 휴대폰 본인확인을 켜면 이 구멍이 같이 막힌다** — 통신사가 확인해 준 번호로만 들어올 수 있기 때문이다.
- **손님은 사진을 올리지 않는다.** 쿠폰은 카운터에서만 나온다. 영수증 업로드·자동 인식·등급(VIP) 코드는 저장소에 남아 있지만 어느 화면에서도 쓰지 않는다.
- **관리자 초기 계정**: 운영에서는 `ADMIN_INITIAL_PASSWORD` 를 반드시 10자 이상의 새 값으로(`Seomyeon2026owner` 처럼). `admin1234` 같은 흔한 값이면 총괄 계정을 만들지 않고 `/admin/login` 에 그 이유를 띄운다 — 손님 사이트는 그대로 열린다. 배포 후 첫 로그인에서 직원 계정을 만들고, 총괄 비밀번호는 `scripts/create-admin.ts` 로 바꿀 수 있다. 관리자 로그인은 IP당 10분 10회, 계정당 15분 8회로 제한된다.
