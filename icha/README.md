...
신분증을 안 가져온 손님을 카운터에서 확인하는 도구다. **로그인·쿠폰·사이트 이용과 아무 상관이 없다** —
이 화면만 혼자 돌아가고, 아무도 막지 않는다. 관리자 → 카운터 오른쪽 위 [성인 확인] 에서 열린다.
**키도 계약도 필요 없다.** 배포하면 바로 쓴다.

직원 폰에서 `/adult`:

1. [검증앱 열기] → 행정안전부 **모바일 신분증 검증앱**(무료) 스토어 화면. 깔려 있으면 [열기].
   아이폰은 App Store(id1600615242), 나머지는 Google Play(kr.go.verify.mobileid)로 간다.
2. 검증앱에서 요청할 정보로 생년월일(운전면허증이면 사진도)을 고르고, 손님이 폰에 띄운 모바일 신분증 QR 을 찍는다
   (정부 앱·삼성월렛·네이버·토스·카카오뱅크·KB·NH 등). 손님이 자기 폰에서 제공을 승인하면 검증앱이 진위를 확인하고
   고른 정보를 보여 준다. 사진과 얼굴을 대조한다.
   PASS 앱 안의 신분증이면 직원 폰 PASS 앱 → 'QR인증'으로 찍는다(진위·성인 여부가 뜬다).
3. 검증앱에 뜬 출생연도 네 자리를 치면 성인 / 미성년 한 글자로 뜬다. 입력한 연도는 서버로 가지 않는다.
   이 판은 테두리만 있다 — 사이트가 확인한 게 아니라 친 숫자로 계산만 한 것이라, 휴대폰 본인확인 결과(꽉 찬 색판)와 모양을 다르게 뒀다.
   판정이 떠 있는 채로 칸을 다시 누르면 비워진다(다음 손님).

왜 사이트가 QR 을 직접 안 읽나 — QR 에는 30초짜리 거래 번호와 조폐공사 중계 서버 주소만 들어 있고,
신원 정보는 한국조폐공사가 심사·승인한 검증자에게만 암호화돼 온다. 승인 없이 읽을 방법이 없다.
그래서 진위 확인은 정부 검증앱이 하고, 사이트는 앱을 열어 주고 연도를 판정한다.

지켜야 할 것:
- **화면만 보고 통과시키지 않는다.** 2026년에 정부24·PASS 화면을 흉내 낸 가짜 앱이 팔리고 있다. QR 을 직원 폰의 공식 앱으로 찍어야 걸러진다.
- 손님 폰에 뜬 "확인 결과" 화면은 믿지 않는다. 찍는 쪽도, `/adult` 를 여는 쪽도 언제나 직원 폰이다.
- 실물도 모바일 신분증도 없으면 이 방법으로는 확인할 수 없다(모바일 신분증은 실물 IC 카드를 대거나 주민센터·면허시험장에 가야 발급된다).

성인 기준은 만 나이가 아니라 연 나이다(청소년 보호법 제2조 제1호 — "19세가 되는 해의 1월 1일을 맞이한 사람은 제외한다").
2026년이면 2007년생까지 통과한다. 판정은 서버 시간대가 아니라 한국 시각으로 센다 — `src/lib/identity/adult.ts`.
모바일 주민등록증은 실물과 효력이 같고(주민등록법 제24조의2), 모바일 운전면허증도 같다(도로교통법 제85조의2).
청소년 보호법 제29조④도 나이를 확인하는 증표로 "주민등록증(모바일 주민등록증을 포함한다)"을 적는다.

### 덧붙일 수 있는 것: 휴대폰 본인확인 (선택)

포트원 + 다날 계약이 있으면 같은 화면 아래에 [휴대폰 본인확인] 버튼이 하나 더 생긴다(통신사를 고르고 생년월일을 넣는 그 창).
이것도 직원 폰에서 연다 — 손님은 인증창에 자기 정보를 넣고, 인증 문자는 자기 폰으로 받는다.
환경변수가 비어 있으면 버튼 자체가 안 보인다. 결과 화면에는 확인 시각과 흘러가는 초가 뜨고 3분이 지나면 "다시"로 바뀐다.
이름·생년월일·CI·DI 는 판정에만 쓰고 저장하지 않는다.

- 신분증 확인은 아니다 — 사진이 없고, 부모 폰으로도 통과된다. 모바일 신분증이 먼저다.
- [admin.portone.io](https://admin.portone.io) 가입(사업자등록증) → **다날** 휴대폰 본인확인 계약(검수 2~5일, 개시까지 7~10일,
  등록비 110,000원 + 월 5만원 1,200건부터, 초과 건당 50원, VAT 별도). *KG이니시스는 쓰지 말 것* — 민간인증서 창이 뜬다.
- 포트원 콘솔에서 채널 속성을 **'본인인증'**으로 골라 추가 → `PORTONE_STORE_ID` / `PORTONE_CHANNEL_KEY` / `PORTONE_API_SECRET` / `PORTONE_IDENTITY_PG=danal`
- 로컬에서 흐름만 보려면 `PORTONE_IDENTITY_MOCK=1`. `NODE_ENV=production` 이면 무시된다.

## 운영 주의
- **본인확인을 붙이기 전에는 전화번호만으로 로그인**한다(인증번호 없음 — 사장님 결정). 그래서 남의 번호를 아는 사람이 그 번호로 들어와 쿠폰함을 보고, 길게 눌러 쿠폰을 "사용" 처리해 버릴 수 있다. 완화책: 번호당 로그인 1시간 10회·IP당 10분 30회 제한, 쿠폰 사용은 매장 직원 앞에서만 인정(직원은 관리자 → 쿠폰 조회에서 코드로 확인하고 사용 처리하는 것을 기본 동선으로).
