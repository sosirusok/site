...
손님 폰에서 `/adult` → [휴대폰으로 확인] → 통신사 본인확인창 → 결과 한 글자(성인 / 미성년).
결과 화면에는 확인 시각과 흘러가는 초가 같이 뜨고 3분이 지나면 "다시"로 바뀐다 — 캡처해 둔 화면을 다시 보여 주는 걸 막는다.

**아무것도 저장하지 않는다.** 이름·생년월일·성별·통신사·CI·DI 는 판정에만 쓰고 버린다. DB 에 남는 줄이 없다.

### 켜는 법

환경변수 세 개가 비어 있으면 이 화면만 "아직 연결되지 않았습니다"라고 뜨고, 사이트의 나머지는 아무 영향이 없다.

1. [admin.portone.io](https://admin.portone.io) 가입 (사업자등록증 필요, 개인사업자 가능)
2. **다날** 휴대폰 본인확인 계약 — 검수 2~5일, 개시까지 7~10일.
   등록비 110,000원 + 월정액(5만원 1,200건부터, 초과 건당 50원, VAT 별도).
   *KG이니시스는 쓰지 말 것* — 통신사 창이 아니라 네이버·카카오 같은 민간인증서 창이 뜬다.
3. 포트원 콘솔에서 채널 속성을 '결제'가 아니라 **'본인인증'**으로 골라 채널 추가
4. `PORTONE_STORE_ID` / `PORTONE_CHANNEL_KEY` / `PORTONE_API_SECRET` / `PORTONE_IDENTITY_PG=danal`

성인 기준은 만 나이가 아니라 연 나이다(청소년보호법 제2조 제1호 — "만 19세가 되는 해의 1월 1일을 맞이한 사람은 제외").
2026년이면 2007년생부터 통과한다. 판정은 서버 시간대가 아니라 한국 시각으로 센다 — `src/lib/identity/adult.ts`.

로컬에서 흐름만 보려면 `PORTONE_IDENTITY_MOCK=1`. `NODE_ENV=production` 이면 무시되므로 운영·미리보기에서는 켜지지 않는다.
