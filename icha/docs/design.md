...
## 2. 디자인 언어 v3 — "실제 매장 안내 사이트" (v1 종이/세리프, v2 어두운 포스터 모두 폐기)

사장님 지적: "표어 같은 감성 문구 넣지 마라. 둥근 사각형 HTML 느낌의 AI 티를 완전히 빼라. 배경색이 이상하다. 진짜 사람이 만든 매장 사이트처럼." 그래서 v3 는 **꾸미지 않는다**. 실제 한국 음식점·프랜차이즈 홈페이지(아임웹으로 만든 매장 사이트)처럼 정보를 정확하게 보여 주는 것이 목표다.

- 바탕 **흰색**, 글자 검정(`--fg`), 보조 회색, 선 `--line`. 강조는 **빨강 `--red` 하나**(무료 표시·주요 버튼). 헤더는 흰색, 푸터는 검정. 어두운 면은 사진 위 히어로와 푸터뿐.
- **모서리 각지게(border-radius: 0)**. 알약(pill) 태그·둥근 카드·그림자 카드·테두리 박스 나열 금지. 구획은 1px 선, 여백, 표(`.table`), 정의목록(`.dl`)으로 나눈다.
- 활자는 Pretendard 만(제목 800, 본문 400/600). Black Han Sans·세리프·큰 표어 활자 금지. 코드·영수증 숫자만 IBM Plex Mono(`.mono`).
- **문구는 사실만, 표어 금지.** "1차는 마음대로", "한 접시 얹어드립니다", "아직 버리지 마세요", "많이 마신 만큼 대접받아요", "네이버에 남긴 진짜 리뷰" 같은 감성 표어·말장난 금지. 섹션 제목은 명사형: 이용 방법 / 참여 매장 / 무료 사이드 메뉴 / 오시는 길 / 등급 혜택 / 이용 안내 / 고객 리뷰. 본문은 합쇼체("~합니다")로 통일. 마키(흐르는 띠), 카운트업, 도장, 테이프 등 연출 금지.
- 사진: 실사진을 **사각형**으로, 오버레이는 히어로에서만(`.dim`). 아이콘·일러스트·이모지 없음. 사진 없는 자리는 만들지 않는다.
- 위치 정보는 `src/lib/locations.ts` 의 값만 쓴다(`subway`, `directions`, `floor`, `parking`, `subwaySummary()`). 좌표로 도보 시간을 따로 계산해 다른 숫자를 보이지 않는다. 지도는 `StoreMap`(실제 지도; 흰 바탕이므로 OSM 밝은 타일 그대로).
- 움직임: 스크롤 등장 `.rise` 정도만. 히어로 사진은 단순 페이드 슬라이드(점 인디케이터) 허용.
- 영수증 인증 결과·쿠폰: 흰 종이 + 1px 검정 테두리(`.paper`), 상태는 굵은 글자(`.status .status-ok/-wait/-no`), 코드는 큰 mono. 톱니 가장자리·도장·프린터 연출·찢기 연출 금지. 쿠폰 사용은 [사용 처리] 버튼 → 확인 창("직원이 확인했습니까?") → 사용 완료 화면(현재 시각 초 단위 표시, 스크린샷 재사용 방지 문구).
- 손님 페이지에 QR 코드를 넣지 않는다. QR 은 관리자 > 포스터(매장에 붙이는 인쇄물)에만 있다.
- **금지 목록**: 둥근 모서리, 그라데이션, 유리 효과, 그림자 카드, 파스텔·금색, 이모지, 아이콘 라이브러리, Tailwind, 12px 이하 글자, 영어 장식 문구, 표어형 카피, 감탄사, "완벽한/특별한/최고의".
