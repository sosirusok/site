...
## 2. 디자인 언어 v6 — "네온 사인" + 사장님이 정한 기능 범위

사장님(카타르시스)과 개발자 대화로 확정된 것:
- **홈페이지의 주목적은 네이버 플레이스 트래픽.** 포스터 QR 은 플레이스로 가고, 플레이스 소개글에 이 홈페이지 링크가 있다. 홈페이지 곳곳에 **플레이스로 가는 초록 버튼**(플레이스 열기·리뷰·길찾기, `src/lib/naver.ts` 의 m.place 주소)을 둔다. 하단 탭 "플레이스"와 `<PlaceButton>` 은 세 매장 시트(`PlaceSheet`)를 연다.
- **영수증 사진·AI 인식·VIP 등급은 뺀다.** 대신 **카운터 발급**: 계산할 때 직원이 손님 휴대폰 번호를 관리자 화면(`/admin/counter`)에 넣으면 릴레이 쿠폰이 생긴다(`src/lib/counter.ts`). 손님은 쿠폰함에서 50m 안 다른 매장 혜택을 고르고, 그 매장에서 직원 앞에서 사용하거나 직원이 번호로 조회해 사용 처리한다. 사진 업로드 화면(/verify)·등급 카드·누적 금액·반려 사유는 손님 화면에서 사라진다.
- **분위기는 사장님 포스터(`public/images/event/poster.jpg`)**: 밤거리, 네온, 굵은 붓글씨(Black Han Sans, 노랑 `--yellow`), 매장색 네온(도쿄 파랑 `--tokyo`, 조선 빨강 `--joseon`, 와르르 초록 `--wareureu`). "핫하고 네온 사인, 클럽 분위기". 사장님이 이 분위기의 그림을 나중에 보내면 자리를 바꿔 넣는다 — 지금은 포스터와 실사진을 쓴다.
- **구조는 v5 그대로(앱 문법)**: 480px 한 단, 목록 행·카드·큰 버튼·하단 탭, 글 짧게(제목 2~8자, 문단 2줄), 해요체, 기술 용어 금지, 터치 44px, 첫 화면에 핵심 한 줄과 버튼.

토큰·유틸(globals.css): 바탕 `--bg #0b0a12`, 카드 `--bg-2`, 글자 `--fg/--fg-2/--fg-3`, 제목 `.h1-event`(노랑 네온) `.h2-event`, 네온 글자 `.neon`(data-store 로 매장색), 네온 테두리 카드 `.card-neon`, 플레이스 버튼 `.btn-naver`(초록), 주요 버튼 `.btn`(노랑), 보조 `.btn-secondary`, 칩 `.tag-store .tag-neon .tag-free`, 사진 띠 `.strip`, 지도 타일은 자동으로 어둡게. 그림자·그라데이션 대신 **네온 글로우(text-shadow/box-shadow)만** 강조로 쓴다. 일러스트 자산은 `coupon-<id>` 티켓(쿠폰 화면)·`empty-pocket`·`how-1..3` 정도만, 어두운 바탕에 어울리게 작게.
