...
## 2-00. v13 — 실제 프랜차이즈·이벤트 사이트 규격 (현재 기준, 아래 v10~v12 는 폐기)

근거: 2026-09-19 실존 사이트 21곳 조사(`scratchpad/refs/checklist.md`·`recommend.md` — 네이버 플레이스 매장 상세, 역전할머니맥주, 한신포차, 생활맥주, 당나발포차, 메가커피, 빽다방, 센텀맥주축제, 부산수제맥주페스티벌, 한강페스티벌, 던킨·맥도날드·CU 이벤트 등).
공통점을 그대로 따른다:

- **구조**: 흰 헤더 56px(로고 + 로그인/내 번호) → 포스터 KV 히어로 1장(`poster-top.jpg`, 글자·버튼은 HTML) → 흰/연회색 섹션 교차(참여 매장 카드 3 → 이용 방법 4단계 → 지도·오시는 길 → FAQ 3개) → 연회색 푸터(상호·주소·사업자등록번호·전화·주류 경고·저작권) → 흰 하단 탭 4개(홈·쿠폰함·예약·안내).
- **매장 화면**: 사진 16:10 → 배지(차수·술·영업 상태)·상호·소개·별점·영업시간·주소·[네이버 예약하기][네이버 플레이스] → 쿠폰 혜택(노란 상자 + 품목 줄) → 사진 격자(2열, 4:3) → 메뉴 목록(사진 56 · 이름 · 설명 · 값, 8개 뒤 접힘) → 지도 카드 + 정보 표 → 리뷰 → 다음 매장 카드. 아래 고정 바 [예약하기][길찾기][전화] 하나(고정 바가 있는 화면은 탭 바를 숨긴다).
- **글꼴**: Pretendard 하나(굵기 400/600/700/800). 제목 26/22/17, 본문 15, 보조 13, 배지 12, 탭 11. Do Hyeon·손글씨 없음.
- **색**: 바탕 흰·연회색(#f5f5f7) 두 단계, 글자 검정 세 단계(#17171a·#4e4e56·#66666f, 모두 4.5:1 이상), 강조 노랑(#ffd400: 배지·혜택 상자·번호 원), 예약 초록(#0d7f3f 흰 글자 5:1), 매장색(파랑·빨강·초록)은 차수 배지·지도 핀·쿠폰 카드 왼쪽 띠에만.
- **형태**: 카드 모서리 14px·테두리 1px, 버튼 한 모양(48px, 색만 primary/naver/outline/soft), 그림자·기울임·외곽선 글자·사선 띠·테이프·도장·종이 없음. 아이콘은 선 SVG 24px.
- **키트 그림 사용처**: `title.png`(헤더 로고 30px), `event/poster-top.jpg`(홈 히어로), `event/poster.jpg`(안내 페이지 포스터 카드), `share.jpg`(공유). 나머지 키트 조각(간판·혜택 상자·버튼·라벨·메모·컷아웃·도장·티켓·리본·순서·테이프·화살표)은 화면에 쓰지 않는다(`docs/kit.md` 참고).
- **성능**: 홈·매장·안내·로그인은 ISR 60초 정적, 세션은 클라이언트 헤더가 `/api/auth/me` 로 확인, 지도(Leaflet)는 화면 근처에서만 로드, 첫 화면 밖 사진 lazy.

## 2-0. v12 — 포스터 한 장처럼 겹쳐 짠다(카드·표·행 없음)

> v12(2026-09, 사장님 피드백 "배경만 핫하고 나머지는 앱처럼 쌓아 놓았다"): 손님 화면은 **포스터의 디자이너가 짠 한 장면**처럼 읽혀야 한다. 조각은 일부러 겹치고, 살짝 기울고, 서로 닿는다. 사진은 간판 **아래**에 깔리고, 컷아웃은 상자 밖으로 **뚫고 나오고**, 스티커가 모서리를 덮는다.

- **좌표 단위 `--u`** = `min(100vw, 480px) / 390`(`.app`). 홈 장면·매장 첫 장면의 모든 좌표는 390 화면 기준 px × `--u`(`calc(200 * var(--u))`) — 430·480 화면에서 비례로 커진다. 글자 크기는 px 그대로.
- **홈 장면(매장마다, 390 기준)**: z0 대표 안주 사진 390×260(아래 30% 가 밤으로 녹음) → z2 간판 358×149 top 200 −2°(왼쪽 위 테이프) → z3 컷아웃 top 140 오른쪽 6 6°(맥주·소주 120px 높이, 주전자 116px 폭 top 112) → z1 폴라로이드 168 top 322 오른쪽 12 3°(사진 창 154×126, 캡션은 혜택 상자 아래로 드러난다) → z2 혜택 상자 300×90 top 362 1.5° + z3 무료 도장 56 top 340 left 290 −12°(3차는 요거트가 상자 왼쪽 위) → 콜아웃(노란 Do Hyeon 18px 외곽선, −3°, 196px 안 두 줄) top 462 → 검은 띠 358×34 top 508(영업 상태 노랑 · 시간) → [예약하기] 52 top 548 −2° · [사진·메뉴 더 보기] 40 top 554 2°. 사진 모서리에 메모 스티커(1차 note-again 오른쪽 위, 2차 note-today 왼쪽 위, 3차 note-good 왼쪽 위). 콜아웃·띠·버튼은 흐름이라 콜아웃이 세 줄이면 아래가 밀린다. 장면 사이는 **검은 띠 44px**(화살표 90° + 노란 "다음 매장 · 2차 조선칼국수 · 도보 2분"), 이벤트 리본은 3차 장면 끝을 20px 덮는다.
- **정보는 포스터식**: 영업 상태·시간·별점·주소·다음 매장·규칙은 **검은 띠(`.band`, `.info`)** 위 Do Hyeon 16px(노랑·흰·크림), 혜택 문장은 **노란 콜아웃(`.callout`)**. 크림 카드 위 상태 칩(`.chip`)·정보표(`.kv`)·크림 다음 매장 줄은 손님 화면에서 쓰지 않는다. 크림 종이는 메뉴판·주소 메모·자주 묻는 질문처럼 '종이'인 것만, 섹션당 한 장, 살짝 기울여서.
- **매장 화면**: 대표 안주 사진 390×300 → 간판 top 250 −2° → 컷아웃 top 190 → 검은 띠(상태 · 오늘 시간 · ★ 별점 / 주문 마감·다른 요일 / 주소, 오른쪽 아래 모서리에 전화 꼬리표 스티커). 특별 혜택은 제목판이 상자 왼쪽 위를 덮고 도장은 왼쪽 아래, 컷아웃은 오른쪽(상자 글자를 가리지 않는 자리), 품목마다 검은 띠(사진 컷아웃이 띠 왼쪽 끝을 뚫는다 · 무료 도장 오른쪽). 사진 벽은 다섯 폴라로이드가 10~16px 씩 겹치되 캡션은 항상 위 카드. 메뉴판은 테이프 붙인 종이(−0.8°). 지도 종이 위 메모 한 장(2°, 지도 오른쪽 아래 모서리를 덮음). 리뷰 조각은 18px 겹침.
- **탭 바**: 포스터 아래 검은 띠. 키트 아이콘(검은 선)은 크림 스티커 네모(30px) 위에, 글자 크림 Do Hyeon, 고른 탭은 노랑. 검은 띠·탭 바 가장자리에는 노랑·검정 사선 띠(`.hz`).
- **검수**: `scratchpad/pw/q1-geo-check.mjs` — 겹침은 글자-글자만 실패(그림 위 그림·글자 아래 그림은 의도), 외곽선 글자(`.callout`/`.outl` ≥16px)는 뒤 픽셀 대비를 재지 않는다(외곽선이 받침).
