# 포스터 키트 — 그림 61장(등록부는 유지, v13 화면은 로고·포스터·공유 이미지만 씀)

> v13(2026-09-19)부터 손님 화면은 실제 프랜차이즈 사이트 규격(`docs/design.md` 2-00)을 따라 키트 조각을 화면 요소로 쓰지 않아요. 지금 쓰는 그림은 `title.png`(헤더 로고), `event/poster-top.jpg`(홈 히어로), `event/poster.jpg`(안내 페이지), `share.jpg`(공유 미리보기)뿐이에요. 아래 등록부(`npm run kit`)와 파일 규칙은 그대로 남겨 두어 나중에 필요하면 다시 쓸 수 있어요.
