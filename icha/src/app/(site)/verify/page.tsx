      ...
      <div className={styles.grid}>
        <header className={styles.head}>
          ...
          <h1 className={`h1 ${styles.title}`}>
            계산하셨죠?<br /><span className="gold">영수증 한 장</span>이면 돼요.
          </h1>
          ...
          {from && (
            <p className={styles.from}>
              <span className={styles.fromNo} aria-hidden="true">{storeNo(from.id)}</span>
              <span>
        ...
        <div className={styles.uploader}>
          {rules.eventActive ? (
            ...
            <div className={`paper paper-shadow ${styles.paused}`}>
              <p className={styles.pausedTitle}>지금은 영수증을 받지 않아요.</p>
        ...
        <div className={styles.termsWrap}>
          <dl className={styles.terms}>
            <div className={styles.term}>
              <dt className={`num ${styles.termNum}`}>{rules.receiptValidHours}<small>시간</small></dt>
              <dd className={styles.termText}>결제 시각부터 {rules.receiptValidHours}시간 안에 올린 영수증만 받아요.</dd>
            </div>
            <div className={styles.term}>
              <dt className={`num ${styles.termNum}`}>{rules.dailyLimitPerMember}<small>장</small></dt>
              <dd className={styles.termText}>하루에 {rules.dailyLimitPerMember}장까지 인증할 수 있어요.</dd>
            </div>
            {rules.minAmount > 0 && (
              <div className={styles.term}>
                <dt className={`num ${styles.termNum}`}>{rules.minAmount.toLocaleString("ko-KR")}<small>원</small></dt>
                <dd className={styles.termText}>{formatWon(rules.minAmount)} 이상 결제한 영수증이어야 해요.</dd>
              </div>
            )}
            <div className={styles.term}>
              <dt className={styles.termWord}>옆집</dt>
              <dd className={styles.termText}>영수증을 받은 매장에서는 혜택이 없어요. 쿠폰은 나머지 두 곳에서 써요.</dd>
            </div>
          </dl>
