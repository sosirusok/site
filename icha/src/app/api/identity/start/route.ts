...
/**
 * 성인 확인 시작 — 브라우저가 인증창을 열 때 필요한 값을 내려 준다.
 *
...
export async function POST(req: Request) {
  try {
    ...
    await setPending({ ivId });

    // 인증창은 대부분 모바일에서 리다이렉트로 돌아온다. 지금 보고 있는 주소로 돌려보내야
    // 방금 심은 진행 쿠키가 같이 돌아온다 — 다른 도메인으로 보내면 쿠키를 못 받아 인증이 버려진다.
    const redirectUrl = `${new URL(req.url).origin}/adult/done`;

    if (!cfg) {
      // 내 컴퓨터에서만 — 실제 창 없이 흐름만 확인한다
      return json({ ok: true, mock: true, identityVerificationId: ivId, redirectUrl });
    }
