...
/** 직원이 보는 판 — 결과 한 글자와, 방금 확인한 것이라는 증거(살아 움직이는 초). */
export function AdultResult({ adult, verifiedAt, onAgain }: { adult: boolean; verifiedAt: string; onAgain: () => void }) {
  const [now, setNow] = useState(() => Date.now());
  useEffect(() => {
    const t = setInterval(() => setNow(Date.now()), 1000);
    return () => clearInterval(t);
  }, []);
  const seconds = Math.max(0, Math.floor((now - new Date(verifiedAt).getTime()) / 1000));
  const stale = seconds > FRESH_SECONDS;
