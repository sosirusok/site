    ...
    <div className={styles.root}>
      <div className={styles.tabs} role="tablist" aria-label="쿠폰을 쓸 매장">
        {stores.map((s) => (
          <button
            ...
            onClick={() => setTab(s.id)}
          >
            <DrinkIcon drink={s.drink} size={22} />
            <span className={styles.tabName}>{s.shortName}</span>
            <span className={`mono ${styles.tabCount}`}>{s.items.length}</span>
          </button>
        ))}
      </div>
      ...
      {stores.map((s) => (
        ...
        >
          ...
          {s.items.length === 0 ? (
            <p className={`paper ${styles.empty}`}>이 매장은 아직 고를 수 있는 사이드가 등록되지 않았어요. 다른 매장에서 골라 주세요.</p>
          ) : (
                    ...
                    >
                      <span className={styles.photo}>
                        {it.image?.kind === "static" ? (
                          ...
                          <span className={styles.noPhoto}><DrinkIcon drink={s.drink} size={28} /></span>
                        )}
                        <span className={styles.free}>무료</span>
                      </span>
                      <span className={styles.itemBody}>
                        <span className={`serif ${styles.itemName}`}>{it.name}</span>
                        {it.price != null && <span className={`mono ${styles.itemPrice}`}>{formatWon(it.price)}</span>}
      ...
      {selected && !issued && (
        <div className={styles.sheetWrap} role="dialog" aria-modal="false" aria-labelledby={`${id}-sheet-title`} data-store={selected.store.id}>
          <div className={`${styles.sheet} ${busy ? styles.sheetBusy : ""}`}>
            <p className="eyebrow">확인</p>
            <p id={`${id}-sheet-title`} className={`serif ${styles.sheetTitle}`}>
              {selected.store.shortName} · {selected.item.name}<br />무료 쿠폰을 받을게요.
            </p>
            <p className={styles.sheetNote}>영수증 한 장에 쿠폰 한 장이에요. 받은 뒤에는 메뉴를 바꿀 수 없고, 오늘부터 {couponValidDays}일 안에 써야 해요.</p>
            {error && <p className="error" role="alert">{error}</p>}
            <div className={styles.sheetActions}>
              <button ref={confirmRef} type="button" className="btn btn-store btn-lg btn-block" onClick={confirm} disabled={busy}>
                {busy ? "쿠폰 만드는 중" : "받을게요"}
              </button>
              <button type="button" className="btn btn-ghost btn-block" onClick={() => setSelected(null)} disabled={busy}>다시 고를게요</button>
            </div>
          </div>
        </div>
      )}
      ...
      {issued && selected && (
        <div className={styles.issueWrap} data-store={selected.store.id} role="status" aria-live="polite">
          ...
          <div className={styles.issued}>
            <div className={styles.issuedBand}>
              <DrinkIcon drink={selected.store.drink} size={20} />
              <span>{selected.store.shortName}</span>
              <span className={`mono ${styles.issuedKind}`}>무료 사이드</span>
            </div>
            <p className={`serif ${styles.issuedName}`}>{issued.menuName}</p>
