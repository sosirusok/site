...
/** 오시는 길 — 지도 카드(세 핀, 220px) 아래 매장마다 한 줄(차수 배지 · 상호 · 주소 · 출구, 오른쪽 [길찾기]). 맨 아래 네이버 검색 링크. */
export function Directions() {
  ...
  return (
    <Section id="map" eyebrow="Map" title="오시는 길" lead={`${walkLine()} · 세 매장 모두 50m 이내`} alt>
      <div className={`card ${s.mapCard}`}>
        <LazyStoreMap stores={mapStores} compact hidePanel height={220} />
      </div>
        ...
        {ordered.map((st) => {
          ...
          return (
            <li key={st.id} className={s.addr} data-store={st.id}>
              <span className={`badge badge-store ${s.addrNo}`}>{st.course.n}차</span>
              <div className={s.addrBody}>
                <p className={s.addrName}>{st.shortName}</p>
                <p className={`small muted ${s.addrSub}`}>{shortAddress(st.address)} · {LOCATIONS[st.id].subway}</p>
              </div>
      ...
      <a href={naverSearchUrl(SEARCH_QUERY)} target="_blank" rel="noreferrer" className={`link ${s.searchLink}`}>네이버에서 &lsquo;{SEARCH_QUERY}&rsquo; 검색 →</a>
    </Section>
  );
}
