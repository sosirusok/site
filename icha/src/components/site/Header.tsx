 ...
 * 로그인 상태는 서버 HTML 에 없다(손님 화면은 정적 ISR) — 마운트 뒤 /api/auth/me 로 알아내면 내 번호 꼬리표로 바뀐다(useMemberPhone).
 */
  ...
  return (
    <header className={styles.header}>
      <Link href="/" className={`${styles.brand} ${shown ? "" : styles.brandHidden}`} aria-label={`${BRAND.name} 홈`} aria-hidden={shown ? undefined : true} tabIndex={shown ? undefined : -1}>
        <Image src={t.src} alt={t.alt} width={t.w} height={t.h} sizes="130px" quality={70} priority className={t.kit ? styles.brandImg : styles.brandImgPoster} draggable={false} />
      </Link>
      {phone ? (
