...
/** 매장 사진 다섯 장 — 큰 폴라로이드 둘 위에 작은 셋이 겹쳐 붙는다(캡션 없음, 흰 테두리만). 맨 아래는 네이버 사진으로 가는 링크. */
  ...
  return (
    <div className={styles.root}>
      <ul className={styles.fan} aria-label={`${store.shortName} 사진 ${photos.length}장`}>
        {photos.map((p, i) => {
          const alt = store.images.find((im) => im.src === p.src)?.alt ?? `${store.shortName} 사진`;
          return (
      ...
      {photoUrl && <a className={`link-d ${styles.more}`} href={photoUrl} target="_blank" rel="noreferrer">사진 더보기 · 네이버 플레이스</a>}
    </div>
  );
}
