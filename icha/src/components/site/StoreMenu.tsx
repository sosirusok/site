...
/** 메뉴 사진 — 배경을 뺀 PNG 는 연회색 상자 안에 그대로, 사진(JPG)은 둥근 사각형으로 자른다. 없으면 빈 상자. */
export function MenuThumb({ m, size = 56 }: { m: MenuItem; size?: number }) {
...
/** 메뉴 — 사진 56px · 이름(혜택 품목은 노란 배지) · 설명 · 값. 혜택 품목이 맨 위, 8개까지 보인 뒤 나머지는 접힘. 맨 아래 [메뉴 전체 보기](네이버). 값은 DB(listMenu). */
        ...
        <>
          ...
          {rest.length > 0 && (
            <details className={styles.more}>
              <summary className={`btn btn-soft btn-block ${styles.moreBtn}`}>메뉴 더보기 ({rest.length}개)</summary>
              <ul className={styles.list}>{rest.map((m) => <Row key={m.id} m={m} />)}</ul>
            </details>
          )}
        </>
      )}
      {menuUrl && <Button href={menuUrl} variant="outline" block srSuffix={` — ${store.shortName}`}>네이버에서 메뉴 전체 보기</Button>}
