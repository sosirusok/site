...
/** 리뷰 — 손글씨 별점 한 줄, 방문자 인용은 찢은 종이 조각 위에, 작은 글자 링크 둘. 값은 stores.ts 의 실제 리뷰. */
export function StoreReviews({ store, limit = 2, reviewUrl, benefit = null }: { store: Store; limit?: number; reviewUrl: string | null; benefit?: string | null }) {
  ...
  return (
    <div className={styles.root}>
      {r && (
        <p className={`hand hand-w num ${styles.rating}`}>
          <span className={styles.score}>★ {r.score.toFixed(2)}</span> 네이버 방문자 리뷰 {r.count.toLocaleString("ko-KR")}개
        </p>
      )}
      {quotes.length > 0 && (
          ...
          {quotes.map((q, i) => (
            <li key={q.date + q.text.slice(0, 8)} className={`scrap ${styles.quote} ${i % 2 ? styles.quoteR : styles.quoteL}`} style={{ "--r": `${i % 2 ? 1.5 : -1.5}deg` } as CSSProperties}>
              <div className="scrap-in">
                ...
                <p className={`hand ${styles.meta}`}>— 네이버 방문자 · {q.date}</p>
              </div>
            </li>
          ))}
        </ul>
      )}
      {benefit && <p className={`hand hand-y ${styles.benefit}`}>{benefit}</p>}
      {reviewUrl && (
        <div className={styles.links}>
          <a className="link link-w" href={reviewUrl} target="_blank" rel="noreferrer">리뷰 쓰기</a>
          <a className="link link-w" href={reviewUrl} target="_blank" rel="noreferrer">리뷰 더 보기</a>
        </div>
      )}
    </div>
  );
