...
export function TabBar({ stores }: { stores: PlaceSheetStore[] }) {
  ...
  const icon = (d: React.ReactNode) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" aria-hidden="true">{d}</svg>
  );
  const item = (href: string, label: string, d: React.ReactNode, active: boolean) => (
  ...
  return (
    <>
      <nav className={`fixed-col ${styles.bar}`} aria-label="하단 메뉴">
        ...
        {item("/guide", "안내", I.info, path.startsWith("/guide") || path.startsWith("/verify"))}
      </nav>
