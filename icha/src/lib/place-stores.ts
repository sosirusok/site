...
/** 예약 시트에 넘길 매장 목록(1차→2차→3차 순, 플레이스 아이디가 있는 매장만) */
export function placeSheetStores(): PlaceSheetStore[] {
  return [...STORES]
    ...
    .map(({ s, l }) => ({ id: s.id, shortName: s.shortName, name: s.name, drink: s.drink, benefitLabel: s.benefitLabel, course: s.course, home: l.home, review: l.review, directions: l.directions, booking: l.booking }));
}
