...
export const ReceiptOcrSchema = z.object({
  is_receipt: z.boolean().describe("결제가 완료된 영수증(카드 매출전표, 현금영수증, 간이영수증)이면 true. 주문서·빌지·메뉴판·화면 캡처면 false"),
  document_type: z
    .enum(["card_slip", "cash_receipt", "simple_receipt", "order_slip", "screen_capture", "other"])
    .describe("card_slip=카드 매출전표, cash_receipt=현금영수증, simple_receipt=간이/영수증, order_slip=주문서·빌지(결제 전), screen_capture=모니터/휴대폰 화면을 찍은 것, other=그 외"),
  ...
  approval_number: z.string().nullable().describe("카드 승인번호(보통 8자리 숫자). 없으면 null"),
